...
async function renderBoost(root) {
  ...
  const name = state.profile === "sondre" ? "Sondre" : "Stine";
  const raw = msgs[state.boostIndex % msgs.length];
  const msg = `${name}, ${raw}`;
  root.innerHTML = `
    <div class="tile">
...
function updateHeader() {
  const p = state.data.profiles.find(x => x.id === state.profile) || {};
  document.getElementById('header-name').textContent = p.name || 'Investor';
}
